...
func (c *greetServiceClient) PrimesApi(ctx context.Context, in *PrimesRequest, opts ...grpc.CallOption) (GreetService_PrimesApiClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[1], "/greet.GreetService/PrimesApi", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServicePrimesApiClient{stream}
	if err := x.ClientStream.SendMsg(in); err != nil {
		return nil, err
	}
	if err := x.ClientStream.CloseSend(); err != nil {
...
type GreetService_PrimesApiClient interface {
	Recv() (*PrimesResponse, error)
	grpc.ClientStream
}

type greetServicePrimesApiClient struct {
	grpc.ClientStream
}

func (x *greetServicePrimesApiClient) Recv() (*PrimesResponse, error) {
...
func (UnimplementedGreetServiceServer) PrimesApi(*PrimesRequest, GreetService_PrimesApiServer) error {
	return status.Errorf(codes.Unimplemented, "method PrimesApi not implemented")
}
...
func _GreetService_PrimesApi_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(PrimesRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GreetServiceServer).PrimesApi(m, &greetServicePrimesApiServer{stream})
}

type GreetService_PrimesApiServer interface {
	Send(*PrimesResponse) error
	grpc.ServerStream
}

type greetServicePrimesApiServer struct {
	grpc.ServerStream
}

func (x *greetServicePrimesApiServer) Send(m *PrimesResponse) error {
	return x.ServerStream.SendMsg(m)
}
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "PrimesApi",
			Handler:       _GreetService_PrimesApi_Handler,
			ServerStreams: true,
		},
	},
